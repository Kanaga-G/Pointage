...
try {
    ...
    } elseif ($type === 'depart_anticipé') {
        // Justifier un départ anticipé
        $stmt = $pdo->prepare("SELECT id FROM depart_anticipations WHERE employe_id = ? AND date_depart = ?");
        $stmt->execute([$employe_id, $date]);
        if ($stmt->fetch()) {
            $stmt = $pdo->prepare("UPDATE depart_anticipations 
                                  SET justifie = ?, commentaire = ?, justifie_par = ?, updated_at = NOW()
                                  WHERE employe_id = ? AND date_depart = ?");
            $stmt->execute([$est_justifie, $commentaire, $justifie_par, $employe_id, $date]);
        } else {
            $stmt = $pdo->prepare("INSERT INTO depart_anticipations 
                                  (employe_id, date_depart, justifie, commentaire, justifie_par, created_at)
                                  VALUES (?, ?, ?, ?, ?, NOW())");
