...
// Correction : Initialisation dynamique du prénom/nom admin si absent
if (
    (!isset($_SESSION['admin_prenom']) || !isset($_SESSION['admin_nom']) ||
    trim($_SESSION['admin_prenom']) === '' || trim($_SESSION['admin_nom']) === '')
    && isset($_SESSION['admin_id']) && $_SESSION['admin_id']
) {
    $stmt = $pdo->prepare("SELECT prenom, nom FROM admins WHERE id = ?");
...
// --- AJOUT : Récupération notifications et messages pour le dashboard ---
try {
    $notificationsStmt = $pdo->prepare("SELECT n.id, n.titre, n.contenu, n.type, n.lue, n.date, n.date_creation, n.lien FROM notifications n WHERE n.employe_id = ? ORDER BY n.lue ASC, n.date DESC, n.date_creation DESC LIMIT 5");
    $notificationsStmt->execute([$employe_id]);
    $notifications = $notificationsStmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    error_log('Erreur SQL notifications : ' . $e->getMessage());
    $notifications = [];
}
try {
    $unreadNotificationCountStmt = $pdo->prepare("SELECT COUNT(*) FROM notifications WHERE employe_id = ? AND lue = 0");
    $unreadNotificationCountStmt->execute([$employe_id]);
    $unreadNotificationCount = $unreadNotificationCountStmt->fetchColumn();
} catch (PDOException $e) {
    error_log('Erreur SQL count notifications : ' . $e->getMessage());
    $unreadNotificationCount = 0;
}
try {
    $messagesStmt = $pdo->prepare("
        SELECT m.id, m.sujet, m.contenu, m.date_envoi, md.lu, a.prenom AS expediteur_prenom, a.nom AS expediteur_nom
        FROM messages m
        JOIN message_destinataires md ON m.id = md.message_id
        LEFT JOIN admins a ON m.expediteur_id = a.id
        WHERE md.destinataire_id = ?
        ORDER BY md.lu ASC, m.date_envoi DESC
        LIMIT 5
    ");
    $messagesStmt->execute([$employe_id]);
    $messages = $messagesStmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    error_log('Erreur SQL messages : ' . $e->getMessage());
    $messages = [];
}
try {
    $unreadMessageCountStmt = $pdo->prepare("SELECT COUNT(*) FROM message_destinataires WHERE destinataire_id = ? AND lu = 0");
    $unreadMessageCountStmt->execute([$employe_id]);
    $unreadMessageCount = $unreadMessageCountStmt->fetchColumn();
} catch (PDOException $e) {
    error_log('Erreur SQL count messages : ' . $e->getMessage());
            ...
            <div class="dashboard-header card shadow-sm mb-4 p-4 bg-white rounded-4 border-0" style="margin-top: 0 !important;">
                <div class="row align-items-center g-1">
                    <div class="col-md-7 d-flex align-items-center gap-1">
                        <div class="icon-circle bg-primary bg-opacity-10 text-primary d-flex align-items-center justify-content-center me-1" style="width:28px;height:28px;border-radius:7px;">
                            <i class="fas fa-users fs-6"></i>
                        </div>
                        <div>
                            <h2 class="fw-bold mb-1" style="font-size: 11px;letter-spacing:0.5px;">Tableau de bord RH</h2>
                    ...
                    <div class="col-md-5 d-flex justify-content-end align-items-center gap-1">
                        <!-- Notifications -->
                        <a href="admin_notification.php" class="position-relative btn btn-light btn-sm rounded-circle shadow-sm" title="Notifications" style="width:28px;height:28px;display:flex;align-items:center;justify-content:center;">
                        ...
                        <!-- Messagerie -->
                        <button type="button" id="btn-messagerie-popup" class="position-relative btn btn-light btn-sm rounded-circle shadow-sm" title="Messagerie" style="width:28px;height:28px;display:flex;align-items:center;justify-content:center;" data-bs-toggle="modal" data-bs-target="#messagerieModal" data-bs-backdrop="false">
                            <i class="fas fa-envelope fa-lg"></i>
                            <?php if ($unread_messages > 0): ?>
                            <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-warning shadow-sm" style="font-size:0.6rem;">
                                <?= $unread_messages ?>
                            </span>
                            <?php endif; ?>
                        </button>
                        <!-- MODAL MESSAGERIE (chargement AJAX, non bloquant) -->
                        <div class="modal fade" id="messagerieModal" tabindex="-1" aria-labelledby="messagerieModalLabel" aria-hidden="true" data-bs-backdrop="false">
                        ...
                        document.addEventListener('DOMContentLoaded', function() {
                            ...
                            if (messagerieModal) {
                                messagerieModal.addEventListener('show.bs.modal', function () {
                                    ...
                                    body.innerHTML = '<div class="text-center text-muted py-5"><div class="spinner-border text-primary"></div><br>Chargement...';
                                ...
                                messagerieModal.addEventListener('shown.bs.modal', function () {
                                    setTimeout(function() {
                                        var firstInput = messagerieModal.querySelector('input, textarea, select, button');
                        ...
                        <!-- Profil admin -->
                        <div class="dropdown">
                            <button class="btn btn-outline-primary rounded-pill px-1 py-1 dropdown-toggle d-flex align-items-center gap-1" type="button" data-bs-toggle="dropdown" style="font-weight:500; font-size:11px;">
                                <i class="fas fa-user-circle me-1"></i> <?= htmlspecialchars(trim(($_SESSION['admin_prenom'] ?? '') . ' ' . ($_SESSION['admin_nom'] ?? '')), ENT_QUOTES, 'UTF-8') ?>
                            </button>
                ...
                <div class="col-md-6 col-lg-3">
                    <div class="card stat-card total h-100" style="border-radius: 8px; min-height: 50px; background: rgba(67,97,238,0.18); box-shadow: 0 1px 4px rgba(67,97,238,0.08); margin-bottom:10px;">
                        <div class="card-body text-center py-2 px-1">
                            <div class="mb-1">
                                <i class="fas fa-users" style="font-size:1.3rem;color:#4361ee;"></i>
                            </div>
                            <div class="stat-count fw-bold" style="font-size:1.3rem;color:#4361ee;" id="count-employes"><?= (int)$total_employes_stats ?></div>
